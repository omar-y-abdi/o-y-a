...
The filename must not shadow Python's standard-library http package.
"""
import json
import argparse
import http.client
...
for address in (args.base_url, args.analytics_url):
    parsed = urlsplit(address)
    if parsed.scheme != 'http' or parsed.hostname not in ('localhost', '127.0.0.1'):
        parser.error('HTTP mutation checks require a local test server')
BASE = urlsplit(args.base_url).hostname
PORT = urlsplit(args.base_url).port or 80
ANALYTICS_PORT = urlsplit(args.analytics_url).port or 80
ORIGIN = args.base_url.rstrip('/')
ANALYTICS_ORIGIN = args.analytics_url.rstrip('/')
...
def contact_closed():
    ...
    code,h,b=fetch('/api/contact','POST',{'Origin':ORIGIN,'Content-Type':'application/json'},json.dumps({'name':'Test','email':'visitor@example.org','message':'','website':'','token':'test','submission':'6c77eb01-9260-4e9c-8a72-262b520ee1b1'}));assert code==503 and json.loads(b)['ok']==False
