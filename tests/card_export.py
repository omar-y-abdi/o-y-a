...
OUT=Path('artifacts/cards');OUT.mkdir(exist_ok=True)
checks=[]
with sync_playwright() as p:
 b=p.chromium.launch(executable_path=os.environ.get('CHROMIUM_PATH') or shutil.which('chromium'),args=['--no-sandbox'])
 for prefix in ('k','j','p'):
  for i in range(1,7):
   key=f'{prefix}{i:02d}';page=b.new_page(viewport={'width':390,'height':844},reduced_motion='reduce')
   ...
   data=page.evaluate('window.__saved');(OUT/(key+'.png')).write_bytes(base64.b64decode(data.split(',')[1]))
...
print('All 18 card canvases and mobile receipt boundaries passed.')
