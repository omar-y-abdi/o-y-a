...
export async function buildCms({ styles, js }) {
  ...
  const assets = await Promise.all([['/social/omar-yusuf.png', 'Delningsbild'], ['/mail/omar-smile.png', 'Brevillustration'], ['/mail/omar-smile.gif', 'Animerad brevillustration'], ['/apple-touch-icon.png', 'Webbplatsikon']].map(async ([src, name]) => {
    let bytes;
    try { bytes = await readFile(`public${src}`); } catch { return null; }
    const dimensions = imageDimensionsFromData(bytes);
    return { id: `builtin-${name}`, src, name, alt: name, mime: src.endsWith('.gif') ? 'image/gif' : 'image/png', bytes: bytes.length, width: dimensions?.width, height: dimensions?.height, builtin: true };
  }));
  const cards = JSON.parse(await readFile('public/data/cards.json', 'utf8'));
  const seed = { pages: pages.map(({ html, css, project, ...page }) => page), blank, runtime: defaultCopy, assets: assets.filter(Boolean) };
