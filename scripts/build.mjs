...
const js = {};
for (const name of ['privacy','cards','joy','main']) {
...
const css = (await Promise.all(['base','home','pages'].map(name => readFile(`src/styles/${name}.css`,'utf8')))).join('\n').trim();
const cssPath = `/assets/site.${hash(css)}.css`;
await writeFile('dist' + cssPath,css);
const renderers = { home, workshop, about, furl, contact, notFound };
for (const page of routes) {
  ...
  await writeFile(destination,layout(page,content,{css:cssPath, main:`/assets/${js.main}`}));
}
const scriptHashes = routes.map(page => `'sha256-${createHash('sha256').update(structuredData(page)).digest('base64')}'`);
const csp = ["default-src 'self'", `script-src 'self' ${scriptHashes.join(' ')}`, "style-src 'self'", "style-src-attr 'unsafe-inline'", "img-src 'self' data: blob:", "font-src 'self'", "connect-src 'self'", "frame-src 'none'", "object-src 'none'", "base-uri 'none'", "form-action 'none'", "frame-ancestors 'none'"].join('; ');
...
await writeFile('.generated/build.json',JSON.stringify({routes:indexed.map(page => page.path),js,css:cssPath},null,2));
console.log(`Built ${routes.length} static pages and ${Object.keys(js).length} small native modules. No runtime dependencies.`);
