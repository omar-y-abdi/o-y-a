import { readConsent, consentCookie } from './privacy.d1e242f909f2.mjs';
...
if (document.querySelector('[data-machine], [data-bubble]')) {
  import('./joy.927bc830a3a3.mjs').then(module => module.initJoy({ toast, track })).catch(() => {
...
if (document.querySelector('[data-contact-form]')) {
  import('./contact.098e75ee32ed.mjs').then(module => module.initContact()).catch(() => {
    document.querySelector('[data-contact-status]').textContent = 'Formuläret kunde inte starta. Försök ladda om sidan.';
  });
}
if (document.querySelector('[data-memory-grid]')) import('./games.ba33b886d74a.mjs').then(module => module.initGames()).catch(() => {
