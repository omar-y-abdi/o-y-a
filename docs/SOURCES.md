...
Endast det offentliga Furl-projektet används som tidigare projekt. Privata repo som råkade finnas i kontosökningen har inte exponerats. Glädjeverkstaden är byggd för den här webbplatsen, inte presenterad som ett externt kunduppdrag.
...
Detta styr versionsvalet i workflowen. Det bevisar inte att workflowen redan har körts. Playwright 1.57.0 var installerad i arbetsmiljön och är pinnad i `requirements-test.txt`; Chromium-versionen i den lokala granskningen finns i `ENVIRONMENT.json`.

## Visuellt material

Maskinen, symbolerna, vikningen och monogrammet är egen CSS/SVG-utformning i koden. Socialbilden bygger på samma formspråk och maskin. Kortbilder skapas av webbplatsens egen canvas-kod. Ingen av de tidigare genererade portfolio-mockuperna, deras porträtt eller deras påhittade projekt används. Inga typsnittsfiler har lagts i leveransen.
