...
export function renderPage(page, built, { version, preview, project } = {}) {
  let base = layout(page, '', { css: built.styles[page.template] ?? built.styles.home, main: built.main });
  if (preview) base = base.replace(/<script type="application\/ld\+json">[\s\S]*?<\/script>/, '');
  const attributes = `id="top" class="${escape(page.bodyClass)}" data-page="${escape(page.path)}"${preview ? ' data-cms-preview="true"' : ''}`;
...
export async function publicContent(request, env, built, csp) {
  ...
  if (!url.pathname.endsWith('/') && url.pathname !== '/404.html') return null;
  const state = await readPublicPage(env.CMS_DB, url.pathname);
  ...
  return new Response(renderPage(page, built, { version: state.version }), { status: missing ? 404 : 200, headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store', 'Content-Security-Policy': await pageCsp(page, csp), 'X-CMS-Version': String(state.version) } });
}
