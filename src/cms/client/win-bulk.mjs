...
const CMS_FONT=/\bcms-font-([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\b/g;
...
export const WIN_BATCH_ACTIONS=Object.freeze({
  archive:Object.freeze({kind:'transform',label:'Arkivera',clearSelection:true,transform:card=>({...card,state:'archived'})}),
  restore:Object.freeze({kind:'transform',label:'Återställ',clearSelection:true,transform:card=>({...card,state:'active'})}),
  trash:Object.freeze({kind:'transform',label:'Till papperskorg',tone:'danger',clearSelection:true,transform:card=>({...card,state:'trash'})}),
  delete:Object.freeze({kind:'transform',label:'Radera permanent',tone:'danger',clearSelection:true,transform:card=>{if(stateOf(card)!=='trash')throw new Error('Bara vinster i papperskorgen kan raderas permanent.');return null;}}),
  category:Object.freeze({kind:'transform',label:'Byt kategori',value:'category',transform:(card,value)=>{if(!FLAVORS.has(value))throw new Error('Kategorin är ogiltig.');return {...card,flavor:value};}}),
  export:Object.freeze({kind:'export',label:'Exportera JSON'}),
});
...
export function applyWinAction(cards,selected,action,value=''){
  const definition=WIN_BATCH_ACTIONS[action];
  if(!definition||definition.kind!=='transform')throw new Error('Batchåtgärden är ogiltig.');
  ...
  const result=cards.map(card=>ids.has(card.id)?definition.transform(card,value):{...card}).filter(Boolean);
...
export function exportWinPackage(cards,selected){
  ...
  for(const card of picked){
    assertCard(card);const source=(card.design?.html??'')+'\n'+(card.design?.css??'');
    for(const match of source.matchAll(MEDIA))resources.add(match[0]);
    for(const match of source.matchAll(CMS_FONT))resources.add(`/media/${match[1]}.woff2`);
  }
  return {format:'omar-wins/v1',cards:picked,resources:[...resources].sort()};
}
export function planWinImport(existing,pkg,{collision='skip',availableResources=null,makeId=()=>`win-${crypto.randomUUID()}`}={}){
  ...
  if(availableResources){const missing=pkg.resources.filter(resource=>!availableResources.has(resource));if(missing.length)throw new Error(`Importpaketet saknar resurser: ${missing.join(', ')}`);}
